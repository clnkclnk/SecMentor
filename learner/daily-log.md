...
- **UploadForge 靶场（5031）5/5**：
  - L1 无检查直接传 / L2 黑名单大小写 shell.PY / L3 白名单+Apache双扩展名 shell.py.jpg / L4 改Content-Type:image/jpeg / L5 图片马GIF89a+Nginx解析漏洞(avatar.gif/x.py)
  - 安全对照（内容+随机名+隔离）打不动
  - 用 `<?py ?>` 标签模拟 PHP 一句话木马，验证 whoami 执行成功
- **XXE 开始（进阶重新学）**：
  - 第一块：本质（XML实体=变量 / 外部实体=读文件+发请求 = 路径穿越+SSRF合体）
  - 第二块：三种利用形态（有回显读文件 / 无回显OOB盲打参数实体 / 借SSRF打内网）
  - OOB 盲打核心：参数实体 %xxe; + 外部 DTD + 内容拼 URL 带外传回攻击者日志
